                              ...
                              const Point2D &center = {})
{
    ...
    return {narrow_cast<int>(dx * cos - dy * sin + center.x), narrow_cast<int>(dx * sin + dy * cos + center.y)};
}
...
std::vector<Point2D> line_points(const Line2D &line)
{
    ...
    for (auto step = 0; step <= n; step++) {
        points.emplace_back(Point2D {narrow_cast<int>(std::round(x)), narrow_cast<int>(std::round(y))});
        x += xstep;
        y += ystep;
    }
